...
include { runner } from params.rootDir + "/target/nextflow/workflows/well_demultiplex_runner/main.nf"
...
workflow test_wf {
    output_ch = Channel.fromList([
        [
          ...
          barcodesFasta: "2-wells-with-ids.fasta",
    ...
    workflow.onComplete = {
        try {
            ...
            System.err.println("Integration test completed successfully.")
            assert false
            def publish_subdir = file("${params.fastq_publish_dir}/200624_A00834_0183_BHMTFYDRXX")
            assert publish_subdir.isDirectory() 
            def all_files = publish_subdir.listFiles()
            assert all_files.size() == 1
            def fastq_publish_dir = file(all_files[0])
            assert fastq_publish_dir.name.endsWith("_demultiplex_unknown_version")
            ...
            def fastqc_files = fastq_publish_dir.resolve("qc/fastqc").listFiles()
            ...
            assert fastq_publish_dir.resolve("qc/multiqc_report.html").exists()
            def fastq_files = fastq_publish_dir.resolve("fastq").listFiles()
            ...
            assert fastq_publish_dir.resolve("SampleSheet.csv").exists()
        } catch (Exception e) {
